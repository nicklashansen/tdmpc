...
class ReplayBuffer():
	...
	def __init__(self, cfg):
		self.cfg = cfg
		self.device = torch.device(cfg.device)
		capacity = cfg.train_steps + int(1e3) # avoid full buffer
		dtype = torch.float32 if cfg.modality == 'state' else torch.uint8
		self._obs = torch.empty((capacity, *cfg.obs_shape), dtype=dtype, device=self.device)
		self._next_obs = torch.empty((capacity, *cfg.obs_shape), dtype=dtype, device=self.device)
		self._action = torch.empty((capacity, cfg.action_dim), dtype=torch.float32, device=self.device)
		self._reward = torch.empty((capacity,), dtype=torch.float32, device=self.device)
		self._priorities = torch.ones((capacity,), dtype=torch.float32, device=self.device)
		self._eps = 1e-6
		self.idx = 0
	...
	def add(self, episode: Episode):
		...
		self._next_obs[self.idx:self.idx+self.cfg.episode_length] = episode.obs[1:]
		...
		self._max_priority = 1. if self.idx == 0 else self._priorities[:self.idx].max().cpu().item()
		new_priorities = torch.full((self.cfg.episode_length,), self._max_priority, device=self.device)
	...
	def sample(self):
		...
		probs /= probs.sum()
		total = len(probs)
		idxs = torch.from_numpy(np.random.choice(total, self.cfg.batch_size, p=probs.cpu().numpy(), replace=False)).to(self.device)
		weights = (total * probs[idxs]) ** (-self.cfg.per_beta)
		weights /= weights.max()

		obs = self._obs[idxs]
		next_obs = torch.empty((self.cfg.horizon+1, self.cfg.batch_size, *self._obs.shape[1:]), dtype=obs.dtype, device=self.device)
		...
		if not obs.is_cuda:
			obs, next_obs, action, reward, idxs, weights = \
				obs.cuda(), next_obs.cuda(), action.cuda(), reward.cuda(), idxs.cuda(), weights.cuda()
